...
Arquivos: `packages/ui/src/patterns/{acesso-carrinho.tsx,cabecalho-site.tsx}`, `apps/web/src/features/catalogo/components/estado-disponibilidade.tsx`, `apps/web/app/(loja)/layout.tsx`, `apps/web/app/(loja)/carrinho/page.tsx`, `apps/web/src/features/pedidos/components/lista-carrinho.tsx` (+ `carrinho-view.ts` puro p/ T41, conector `lib/carrinho/acesso-conectado.tsx`, `(site)/layout.tsx` p/ montar o provider) · Testes: T41 + critérios visuais 2,3,6,7 · Depende: F · Est: 75min · Agente: `general-purpose` (mapa de tradução: `design.md` §4.4) · `[x]` · **Gate Visual B aprovado pelo PM em 2026-08-19**

### Bloco J — UI do checkout e do pedido
Arquivos: `apps/web/app/(loja)/checkout/page.tsx`, `apps/web/app/(loja)/pedido/[numero]/page.tsx`, `.../components/{resumo-pedido,seletor-endereco,estado-pagamento}.tsx` (+ `checkout-cliente.tsx`, `checkout-view.ts` puro, `features/auth/destino.ts` p/ matcher `/pedido`, `globals.css` serrilha, `packages/ui/components/badge.tsx` tom de status) · Testes: T31 + critérios visuais 1,4,5,8,9,10 · Depende: G, H, I · Est: 90min · Agente: `general-purpose` (mapa de tradução: `design.md` §4.4) · `[x]` · **Gate Visual B aprovado pelo PM em 2026-08-19**
...
- **Bloco I — `<AcessoCarrinho>` presentacional em `packages/ui` + conector no app.** O cabeçalho mora em `packages/ui` e não pode importar `useCarrinho` (feature do app). Solução do design §4.4.3 ("o cabeçalho RECEBE o `<AcessoCarrinho>`"): o UI expõe o componente presentacional (contador por prop), o app injeta a ilha já ligada (`lib/carrinho/acesso-conectado.tsx`) no slot `acessoCarrinho` do `<CabecalhoSite>`. Circularidade resolvida sem o UI depender do app.
- **Bloco I — `CarrinhoProvider` montado em `(site)/layout.tsx` E `(loja)/layout.tsx`.** O `(site)/layout` **entra no toque** (fora da lista nominal do bloco, mas é a própria mudança do Mapa: "primeira vez que o cabeçalho estático lê estado de cliente"): sem o provider ali, o contador e o "Adicionar" das páginas de produto não leem carrinho. Duas instâncias isoladas, uma por grupo de rota, ligadas pelo `localStorage` que atravessa a navegação de página inteira — não precisam compartilhar estado React.
- **Bloco I — lógica da tela extraída para `carrinho-view.ts` (puro), testada em node.** Mesmo motivo do bloco F: o runner é `environment: 'node'` sem jsdom. `montarVistaCarrinho` decide `podeFinalizar` (trava o checkout quando há esgotado — T41) e é testada sem React; o render é auditado no Gate Visual B.
- **Bloco I — a página `(loja)/carrinho` (server) lê o catálogo e entrega à ilha cliente, que revalida no servidor.** O carrinho guarda só id+quantidade (RN1); o catálogo (nome/faixa/foto, estável) vem do server, e preço/vaga vêm de `POST /api/carrinho/validar` (RN3). Frete não aparece (crit. visual 2 — "no próximo passo"); dia derivado com o motivo (crit. 3); esgotado é card que permanece e trava o avanço (crit. 6 + T41); contador some quando vazio (crit. 7).
- **Bloco I — Gate Visual B aprovado ("ficou ótimo", PM em 2026-08-19).** Dois pontos levantados na validação **não** viraram mudança do bloco: (a) escolher a data no carrinho → **NAPO-022** (contradiz a RN2, reabre spec); (b) incoerência do "disponível" por sabor vs. teto compartilhado da fornada → **NAPO-023** (semântica de exibição do NAPO-003/004, não do carrinho). Captura em vez de scope creep (AGENTS §4.3).
- **Bloco J — `middleware.ts` NÃO foi tocado; o matcher de sessão vive em `features/auth/destino.ts`.** O Mapa dizia `middleware.ts`, mas o mecanismo real (herdado do NAPO-002) é o `AREA_POR_SEGMENTO`: `checkout` já estava lá (protegido desde o NAPO-002), e só faltou `pedido → 'conta'`. Corrigido no lugar certo; o middleware chama `rotaExigeSessao` que lê esse mapa.
- **Bloco J — checkout é página server (guarda `exigirAcesso('checkout')`) + ilha cliente.** A guarda de sessão+telefone (RN5) roda antes de renderizar; a orquestração (carrinho + frete + pagamento) é o `checkout-cliente.tsx`. O `<SeletorEndereco>` renderiza o próprio radio (não reusa `<CardEndereco>` — que não tem estado de seleção e é de outra feature); tipo `EnderecoParaCheckout` local evita import cruzado entre features.
- **Bloco J — resumo na direção A ("ficha da fornada") com `.serrilha` virando CSS de produto** (`globals.css`, §4.4.5): dia extrabold > total (crit. 1). No mobile o resumo vira barra fixa no rodapé (crit. 8/T31); `<Badge variant="sucesso">` com data (crit. 5, sem timeline). Lógica de resposta do pagamento extraída para `checkout-view.ts` (pura, testada — E1/E3/gateway).
- **Bloco J — `estado-pagamento.tsx` reconcilia com polling de espaçamento crescente + teto** (RN19/T38); cancelamento via `<Dialog>` (RN15). Página do pedido é server (guarda + `lerPedidoDoDono`), UUID interno não vai ao cliente (número é o id público, RN16).
- **Bloco J — bugs achados no Gate Visual B e corrigidos:** (a) carrinho não esvaziava → `limpar()` na criação do pedido (evita item fantasma e pedido duplicado); (b) flash de "carrinho vazio" → ref `finalizando` guarda o redirect; (c) id do fake não-único colidia no `mp_payment_id` de pedidos de mesmo total → `fake-<centavos>-<numero>` + reconciliação best-effort no GET (commit `9a5fcdb`, blocos E/H).
