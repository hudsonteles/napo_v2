...
- **Túnel de desenvolvimento — a URL muda e o humano precisa saber:** o webhook do Mercado Pago não alcança `localhost`, então dev roda atrás de `cloudflared` (`cloudflared tunnel --url http://localhost:3000`). O túnel grátis **sorteia um endereço novo a cada vez que sobe**. Sempre que o agente subir ou derrubar o túnel, ou perceber que o endereço mudou, deve **avisar o humano explicitamente com a URL nova** e atualizar `DEV_TUNNEL_HOST` em `apps/web/.env.local` — sem isso o Next recusa as requisições vindas do túnel. Se o teste envolver notificação do gateway, lembrar também que a URL cadastrada no painel do Mercado Pago ficou obsoleta. **Não usar ngrok:** o Windows Defender o remove como `Trojan:Win32/Kepavll!rfn`, e a alternativa seria abrir exclusão de antivírus.
- **Commits — conta do GitHub:** este repositório é `hudsonteles/napo_v2`. A máquina tem várias contas no `gh` e a ativa pode não ser a certa — o push então falha com `403 Permission denied`. O agente **tem autorização permanente** para corrigir sozinho, sem perguntar: `gh auth switch -h github.com -u hudsonteles` e repetir o push. Não abra branch, não peça credencial, não deixe o commit parado localmente.
